...
string signed_extension(string s){
	...
	int padding = 16 - size; /* determine how many bits are required for padding. */

	char bit = s[0]; /* copy value of msb. */

	s.insert(0, padding, bit); /* replicate msb for padding. */

    return s; /* return string. */
}

int binary_to_decimal_signed(string s){
	reverse(s.begin(), s.end()); /* reverse string for simpler iteration. */

	int size = s.size();
	int decimal = 0;

	for (int i = 0; i < size; ++i) { /* add decimal value of every bit together. */
		int digit = s[i] - '0'; /* convert character to integer. */

		if (i == size - 1 && digit == 1)
			decimal -= digit * pow(2, i); /* subtract msb if set to true. */
		else
...
string decimal_to_binary_signed(int n){
	int absolute = (n >= 0) ? n : -n; /* store absolute value of n. */
	string result = decimal_to_binary(absolute); /* convert absolute value to binary. */
	result.insert(0, 1, '0'); /* prepend '0' to ensure 'signed_extension' doesn't confuse a positive for a negative. */

	if (n < 0) /* if n is negative, apply two's complement to result. */
		result = twos_complement(result);

	result = signed_extension(result); /* add padding to result. */
    return result;
}

string add_binaries_signed(string b1, string b2){
    /* extend binary numbers. */
	b1 = signed_extension(b1);
	b2 = signed_extension(b2);
	
	string result = add_binaries(b1, b2); /* add them together. */
	result = result.substr(result.size() - 16); /* slice difference to first 16 bits. */
    return result;
}

string twos_complement(string s){
    string extension = signed_extension(s); /* extend binary number (i.e. add padding). */

    for (int i = 0; i < extension.size(); ++i) /* one's complement (inverting values). */
		extension[i] = (extension[i] == '0') ? '1' : '0';

	string complement = add_binaries(extension, "1"); /* two's complement (adding one). */
	complement = complement.substr(complement.size() - 16); /* slice complement to first 16 bits. */
